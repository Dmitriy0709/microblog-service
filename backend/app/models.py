...
class Base(DeclarativeBase):
    __allow_unmapped__ = True  # Позволяет пропускать несовместимые аннотации
...
class User(Base):
    ...
    following: Mapped[List["Follow"]] = relationship("Follow", back_populates="follower", foreign_keys="Follow.follower_id", cascade="all, delete-orphan")
    followers: Mapped[List["Follow"]] = relationship("Follow", back_populates="followee", foreign_keys="Follow.followee_id", cascade="all, delete-orphan")
...
class Tweet(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=False), nullable=False, server_default=func.current_timestamp(), default=datetime.utcnow)
