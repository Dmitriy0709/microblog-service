...
if TYPE_CHECKING:
    # для поддержки ссылок, чтобы избежать циклических импортов
...
class User(Base):
    ...
    # Чтобы mypy не ругался, используйте List и добавьте # type: ignore
...
class Follow(Base):
    ...
    follower: User = relationship("User", back_populates="following", foreign_keys=[follower_id])
    followee: User = relationship("User", back_populates="followers", foreign_keys=[followee_id])
